if game.PlaceId == 142823291 then
  ...
  CloseIcon.MouseButton1Click:Connect(function()
    TweenService:Create(CloseIcon, TweenInfo.new(0.1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
      ImageTransparency = 1
    }):Play()
    wait()
    TweenService:Create(MainFrame, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
      ...
      Visible = false,
      Position = UDim2.new(0, MainFrame.AbsolutePosition.X + (MainFrame.AbsoluteSize.X / 3), 0, MainFrame.AbsolutePosition.Y + (MainFrame.AbsoluteSize.Y / 3))
    }):Play()
    wait(1)
    ScreenGui:Destroy()
  end)
